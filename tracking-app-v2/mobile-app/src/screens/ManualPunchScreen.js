...
// Pure manual screen - only responds to the employee tapping a button.
// Auto-tracking (background/foreground) lives entirely in AutoTrackScreen,
// so the two are never mixed up.
export default function ManualPunchScreen({ user, onPunchStatusChange }) {
  ...
  const handlePunch = async (type) => {
    ...
    try {
      ...
      if (onPunchStatusChange) onPunchStatusChange(type);
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Manual Punch</Text>
      <Text style={styles.subtitle}>{user.name} ({user.employeeId})</Text>
      <Text style={styles.helper}>Punch In starts automatic tracking. Punch Out stops it.</Text>
...
const styles = StyleSheet.create({
  ...
  helper: { fontSize: 12, color: "#64748b", textAlign: "center", marginBottom: 24 },
